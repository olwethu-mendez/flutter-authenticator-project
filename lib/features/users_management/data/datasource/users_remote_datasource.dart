...
abstract class UsersRemoteDataSource {
  ...
  Future<List<UsersFilteredListModel>> getFilteredUsers(
    String? fullName,
    String? email,
    String? phoneNumber,
  );
...
class UsersRemoteDatasource implements UsersRemoteDataSource {
  ...
  @override
  Future<List<UsersFilteredListModel>> getFilteredUsers(String? fullName, String? email, String? phoneNumber) async {
    final Map<String, dynamic> queryParams = {};
    // Only add non-null values (important!)
    ...
    try {
      final response = await dio.get('/users/users-filter', queryParameters: queryParams);
      final List<dynamic> data = response.data;
      return data.map((json) => UsersFilteredListModel.fromJson(json)).toList();
    } on DioException catch (e) {
      _handleError(e);
      rethrow;
    }
